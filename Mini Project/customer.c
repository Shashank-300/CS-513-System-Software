...
// Function to remove a user from the session file
void remove_user_from_session(int userID) {
    ...
    if (file == NULL) {
        printf("Error: Could not open session.txt for reading.\n");
        return;
    }

    // Open a temporary file for writing
    tempFile = fopen("temp_session.txt", "w");
    if (tempFile == NULL) {
        printf("Error: Could not open temp_session.txt for writing.\n");
        fclose(file);
        return;
    }

    // Read through the session file and copy all user IDs except the one to be removed
    while (fscanf(file, "%d", &sessionUserID) != EOF) {
        if (sessionUserID == userID) {
            found = 1; // User found, so we don't write this userID to the temp file
        } else {
            fprintf(tempFile, "%d\n", sessionUserID); // Write other user IDs to the temp file
        }
    }
    ...
    // If the user was found, replace the session file with the temp file
    if (found) {
        remove("session.txt");                // Delete the old session file
        rename("temp_session.txt", "session.txt"); // Rename temp file to session.txt
        printf("UserID %d successfully removed from session.txt.\n", userID);
    } else {
        // If the user was not found in the session file
        remove("temp_session.txt"); // Delete the temp file as no changes were made
        printf("UserID %d not found in session.txt.\n", userID);
    }
}
...
void transfer_funds(int connFD, int senderID) {
    ...
    // find recipient's account based on the recipient's account ID
...
void add_customer_feedback(int connFD, int userID) {
    ...
    char buffer[600]; // buffer to read feedback input from the user
    ...
    newFeedback.userID = userID;  // Set customer/user ID
    strncpy(newFeedback.feedbackText, buffer, sizeof(newFeedback.feedbackText) - 1); // Copy feedback

    newFeedback.feedbackText[sizeof(newFeedback.feedbackText) - 1] = '\0';

    // Open the feedback.txt file in append mode
    FILE *feedbackFile = fopen("feedback.txt", "ab");
    ...
    fwrite(&newFeedback, sizeof(struct Feedback), 1, feedbackFile);

    // Close the file
    fclose(feedbackFile);

    // Confirm to the customer that their feedback was submitted
