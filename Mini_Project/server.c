...
#include "./admin.h"
#include "./customer.h"
#include "./user.h" // Include for employee operations
#include "./employee.h"
#include "./manager.h"
...
void connection_handler(int connectionFileDescriptor) {
    ...
    // Modify the initial prompt to include Manager and Employee options
    #define INITIAL_PROMPT "Select your role:\n1. Admin\n2. Customer\n3. Manager\n4. Employee\nEnter your choice: "
    ...
    if (writeBytes == -1) {
        ...
        if (readBytes == -1) {
            ...
            case 1:
                // Admin
                admin_login(connectionFileDescriptor);
                break;
            case 2:
                // Customer
                customer_login(connectionFileDescriptor);
                break;

            case 3:
                // Manager
                manager_login(connectionFileDescriptor);
                break;
            case 4:
                // Employee
                employee_login(connectionFileDescriptor);
                break;
            default:
                // Invalid input
                const char *invalidInputMsg = "Invalid choice! Please try again.\n";
                write(connectionFileDescriptor, invalidInputMsg, strlen(invalidInputMsg));
                break;
            }
        }
    }
